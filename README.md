...
This is `3.2.1`: `3.2.0` plus the remote vector companion with its Qdrant backend, the
single rule that decides a remote origin, and install support for Python 3.13. The notes
are the `[3.2.1]` section of [CHANGELOG.md](CHANGELOG.md). Several Hermes agents
...
This fork's `3.2.1` build is installed from a wheel file built out of this branch
